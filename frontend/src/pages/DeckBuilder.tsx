...
export default function DeckBuilder() {
  ...
  const parseFlatImportText = (text: string) => {
    ...
    blocks.forEach((block, index) => {
        ...
        // Check for Sideboard block
        if (firstLineLower.startsWith('sideboard')) {
             // Remove header line if it is not a card (starts with digit)
             // Although "Sideboard" won't match the card regex anyway, we skip it to be clean
            ...
            // Last block and not the only block -> Commander
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh]">
            <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-950/50 rounded-t-xl">
              ...
              <button onClick={() => setShowImportModal(false)} className="text-slate-500 hover:text-slate-300 transition-colors">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-4 border-t border-slate-800 bg-slate-950/50 rounded-b-xl flex justify-end gap-3">
              <button onClick={() => setShowImportModal(false)} className="px-4 py-2 rounded text-slate-400 hover:text-slate-200 hover:bg-slate-800 transition-colors font-bold">Cancel</button>
